...
function generateCommunityPage(community, template) {
  ...
  const keywords = `DevFest, Kaohsiung, ${nameZh}, Community, Google Developer`;
  ...
  html = html.replace(
    ...
    `<meta property="og:url" content="${escapeHtml(communityUrl)}" id="ogUrl" />`
  );
  html = html.replace(
  ...
  html = html.replace(
    ...
    `<meta property="twitter:url" content="${escapeHtml(communityUrl)}" id="twitterUrl" />`
  );
  html = html.replace(
  ...
  html = html.replace(
    ...
    `<link rel="canonical" href="${escapeHtml(communityUrl)}" id="canonicalUrl" />`
  );
